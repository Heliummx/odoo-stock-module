...
class WebhookOdooOrderUpload(http.Controller):
    ...
    def odoo_upload_order(self, **post):
        ...
        try:
            ...
            sale_order_odoo = request.env['sale.order'].sudo().search([('woocommerce_sale_order_id', '=', woo_order_id )])
    ...
    def update_order(sale_order, data):
    ...
    def create_order(self, data):
        ...
        if customerId != 0:
            partner_odoo = request.env['res.partner'].sudo().search([ ('woocommerce_customer_id' , '=', customerId) ])
            if not partner_odoo.exists():
                partner_odoo = request.env['res.partner'].sudo().search([('email', '=', emailCustomer)])
                if not partner_odoo.exists():
                    partner_odoo = self.generate_user(emailCustomer, billing_address )
    ...
    def get_order_lines(self, cart):
        ...
        for cart_item in cart:
            product_id = request.env['product.product'].sudo().search([ ("default_code","=", cart_item['sku']) ])
            if product_id:
                res.append((0,0,{
